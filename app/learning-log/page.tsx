...
export default function LearningLogPage() {
  ...
  const calculateCurrentPage = (
    data: Pick<PaginatedResponse<Topic>, "next" | "previous">
  ) => {
    let page = 1;
  ...
  const fetchTopics = useCallback(
    async (url: string = "/learnings/topics/") => {
      ...
      try {
        ...
        const res = await api.get<PaginatedResponse<Topic>>(url, { params });
        const data = res.data;
        ...
        setCurrentPage(calculateCurrentPage(data));
  ...
  const handleAddEntry = async (entry: Omit<LearningEntry, "id">) => {
    try {
      ...
      if (err instanceof AxiosError) {
        toast.error(err.response?.data?.detail || "Failed to add learning");
  ...
  const handleEditEntry = async (id: string, title: string) => {
    try {
      ...
      if (err instanceof AxiosError) {
        toast.error(err.response?.data?.detail || "Failed to update learning");
      } else if (err instanceof Error) {
        toast.error(err.message);
      } else {
        toast.error("Something went wrong");
  ...
  const handleDeleteEntry = async (id: string) => {
    try {
      ...
      if (err instanceof AxiosError) {
        toast.error(err.response?.data?.detail || "Failed to delete learning");
      } else if (err instanceof Error) {
        toast.error(err.message);
      } else {
        toast.error("Something went wrong");
